...
and \\
  ...
  Durham University\\
  Durham, UK \\
  E-mail: \email{amani.m.almohaimeed@durham.ac.uk}\\
\\\, \\
  Jochen Einbeck\\
  Department of Mathematical Sciences\\
  Durham University\\
...
\begin{document}
\Sexpr{opts_knit$set(concordance = TRUE)}
\baselineskip 15pt
...
This algorithm is implemented in \textbf{\textsf{R}} function \code{alldist()}, which is designed to account for simple 
overdispersion models using the NPML estimation, while variance component models can be fitted with \code{allvc()} in the 
\pkg{npmlreg} package \citep{Aitkin2009,Einbeck2014}. For both overdispersed and variance component models, 
the EM algorithm for NPML estimation of the mixing distribution was regarded as ``very stable and converged in every case'' \citep{Aitkin1999}.
...
in overdispersed exponential family models make them a powerful modelling tool''. The ability of the EM algorithm to locate the global maximum in fewer iterations can be affected by the choice of initial values, 
several methods for choosing initial values for the EM algorithm in the case of finite mixtures are discussed by \cite{KarlisXekalaki2003}. 
...
This vignette introduces (an implementation of) a transformation approach by extending the Box-Cox transformation to overdispersion and two–level variance 
...
In this article, we introduce the \pkg{boxcoxmix} package for response transformations for random effect and variance component 
models \citep{Amani}, which is available from the Comprehensive \textbf{\textsf{R}} Archive Network (CRAN) at 
\url{https://cran.r-project.org/package=boxcoxmix}. 
\pkg{boxcoxmix} combines 
the Box-Cox transformation with random effects and variance component models. For an implementation of the Box-Cox transformation 
for the univariate linear model in \textbf{\textsf{R}}, see the \code{boxcox()} function in the \pkg{MASS} package \citep{VenablesRipley2002}. 
...
more closely than the untransformed data).  We 
extend this transformation to \pkg{boxcoxmix}'s random effect and variance component models using the NPPML technique. \\
...
\textbf{E-step:} Adjust weights using the formula
\begin{equation} 
\label{wik}
w_{ik}= \frac{\hat{\pi}_{k} f_{ik}}{\sum_{\ell}\hat{\pi}_{\ell} f_{i\ell}}
\end{equation} \\
where $w_{ik}$ is the posterior probability that observation $y_{i}$ comes from cluster $k$, 
and $f_{ik}$ depends via equation (\ref{fik}) implicitly on the current values of $\hat{z}_{k}$, $\hat{\beta}$ and $\hat{\sigma}^{2}$.\\
\\
...
Replacing the results into Equation (\ref{eq: lr}) we get the non-parametric profile log-likelihood function
\begin{equation}
\ell_{P} (\lambda)= \sum^{n}_{i=1} \sum_{k=1}^{K} \Bigg[ G_{ik} \log \hat{\pi}_{k} + G_{ik} \log \hat{f}_{ik}\Bigg].
 \end{equation}
...
In package \pkg{boxcoxmix}, the main function for fitting random effect models with response transformations is \code{optim.boxcox()}, which 
 performs a grid search over the parameter $\lambda$ and then optimizes over this grid, 
in order to calculate the maximum likelihood estimator of the transformation. It produces a plot of the non-parametric profile likelihood function that summarises 
information concerning $\lambda$, including a vertical line indicating the best value of $\lambda$ that maximizes the non--parametric profile log--likelihood.  
In order to fit models with fixed value of $\lambda$, one can use function \code{np.boxcoxmix()} that is designed to account for `overdispersed' 
linear models and variance component models.  When $\lambda$ =1 (no transformation), the results of the proposed approach will be very similar to that of the \pkg{npmlreg} 
...
including non-normal data and the number of classes, $K$. Additionally, it produces the parameter estimates, the standard errors of the estimates and the 
log-likelihood value. Further, the Akaike Information Criterion (AIC) is calculated to find the best fitting line for the data, using the following formula
\begin{equation} \label{aic}
\mbox{AIC} = -2 \log  L +2\times (p+2K)
\end{equation}
where $p$ is the number of parameters in $\beta$ and $K$ is the number of mixture classes.
...
\begin{equation}
z_{k}^0= \beta_{0}+ \emph{tol} \times s \times g_{k}
\end{equation}
...
\end{equation}
where $n-p$ is the degrees of freedom for $\hat{\varepsilon_{i}}$, $n$ is the sample size, $p$ represents the number of parameters used 
to fit the model $y_{i}^{(\lambda)}=x_{i}^{T}\beta + \varepsilon_{i}$ and $\hat{\varepsilon_{i}}$ is the difference between the observed 
data of the dependent variable $y_{i}^{(\lambda)}$ and the fitted values $\hat{y_{i}}^{(\lambda)}$(i.e. $\hat{\varepsilon_{i}}=y_{i}^{(\lambda)}-\hat{y_{i}}^{(\lambda)}$).
...
\end{itemize}
(For either case, \pkg{boxcoxmix} provides the function \code{tolfind.boxcox()} to identify optimal values of \emph{tol}.) 
...
can be applied on the output of \code{np.boxcoxmix()}, \code{optim.boxcox()} and \code{tolfind.boxcox()}.
...
\item 2, the fitted value against the response of the untransformed and the transformed data; 
...
For a fixed value of $\lambda$,   the model can be fitted  using 
function \code{np.boxcoxmix()} with settings $\lambda=-1$,  $tol=1.8$ and $K=3$ (the latter two choices to be justified below), so the response will be 
transformed as 
\[
...
transformed model using $\lambda =-1$ and $\hat{\lambda}=0.1$, and the untransformed model ($\lambda =1$). 
...
$\delta_{2,5}$ & -0.1923&0.2595&0.3378& 0.3536   \\
 ...
 $-2 \log L$ &-68 &-73.7& -98 & -86.6 \\
   AIC & -38& -43.7085 &-68.0224&-56.6193 \\
 \hline
\end{tabular}
...
The parameter estimates of the untransformed and the Box--Cox--transformed model using $\hat{\lambda}$ are in broadly agreement but the latter has better 
...
\begin{tabular}{ cccc  }
 K & $\lambda =-1$   &$\hat{\lambda}=0.1$ &$\lambda=1$  \\
   \hline
 1 & -28.0144 &  -31.5791 &  -27.4505\\
   2   &  -48.1073 & -54.7102  &-42.6445 \\
  3 &   -43.7085 & -68.0224 &-56.6193 \\
  4 &  -48.4297 &  -57.4002& -50.4271\\
5&  -55.4437 &  -58.1701 & -47.1773\\
6&  -62.5389 &  -49.4002 & -42.4272\\
7&  -47.4436  &   -50.1702 & -52.3925\\
 \hline
\end{tabular}
...
\begin{eqnarray*}
\hat{\beta}^{(\lambda)}&=&\left(  \sum^{r}_{i=1}  \sum_{j=1}^{n_{i}} x_{ij} x_{ij}^{T} \right)^{-1} \sum^{r}_{i=1} \sum_{j=1}^{n_{i}} x_{ij} 
\left(  y_{ij}^{(\lambda)} - \sum_{k=1}^{K} w_{ik} \hat{z}_{k} \right),\label{eq: betav1}\\
\hat{\sigma}^{2 (\lambda)} &=& \frac{\sum^{r}_{i=1} \sum_{k=1}^{K} w_{ik} \left[\sum_{j=1}^{n_{i}}(y_{ij}^{(\lambda)} - x_{ij}^{T} \hat{\beta}-\hat{z}_{k})^{2}\right]}{\sum^{r}_{i=1} n_{i}}, \\
\hat{z}^{(\lambda)}_{k} &=& \frac{\sum^{r}_{i=1} w_{ik}\left[ \sum_{j=1}^{n_{i}} (y_{ij}^{(\lambda)} - x_{ij}^{T} \hat{\beta})\right]}{\sum^{r}_{i=1} n_{i} w_{ik}}, \label{eq: zv1}\\
 \hat{\pi}^{(\lambda)}_{k} &=&\frac{\sum^{r}_{i=1} w_{ik}}{r},
\end{eqnarray*}
where $\hat{\pi}^{(\lambda)}_{k}$ is the average posterior probability for component $k$.  Substituting the results into Equation (\ref{eq: 9}) we get the non-parametric profile log-likelihood function 
\begin{equation}
 \ell_{P} (\lambda)= \sum^{r}_{i=1} \sum_{k=1}^{K} \Bigg[ G_{ik} \log \hat{\pi}_{k} + G_{ik} \log \hat{m}_{ik}\Bigg].
\end{equation}
The non--parametric profile maximum likelihood (NPPML) estimator is therefore given by
\begin{equation}
\hat{\lambda} = arg  \, \max_\lambda \: \ell_{P} (\lambda).
...
Fitting variance component models using response transformations with an unspecified mixing distribution also can be done in \textbf{\textsf{R}}
using the \pkg{boxcoxmix} function \code{np.boxcoxmix()}. When $\lambda$ =1 (no transformation), the results of the proposed 
...
\begin{figure}
<<fig.keep='high', eval=TRUE, echo=TRUE, out.width='60%'>>=
plot(testox, 1)
@ 
...
The manual specification of \code{Oxboys$boy <- gl(26,9)} is necessary since the second argument of \code{np.boxcoxmix} requires a vector of group labels in order to work correctly.
...
The following results of the data before and after applying the response transformation shown in Table  \ref{table:boy} 
...
\begin{tabular}{ cc cc cc cc c  }
 ...
 & $\hat{\lambda}=0.1$   &$\lambda =1$& $\hat{\lambda}=-0.51$   &$\lambda =1$ & $\hat{\lambda}=-0.25$   &$\lambda =1$  & $\hat{\lambda}=-0.25$   &$\lambda =1$\\
   \hline
 NPPML 		&-605.9 &-606.4&-559.7 &-566.5&-513.3&-Inf&-512.4&-566.5\\
$\hat{\beta}$ 	&0.0716	&6.5264 & 0.0034 &6.5218 &0.0126 & 6.5245  &0.0126&6.5218\\
$SE(\hat{\beta})$ &0.0031 &0.2841 & 0.0001 &0.2367 &0.0004 & 0.1918  &0.0004&0.2367\\
 $\hat{\sigma}$ &0.0310 & 2.806 & 0.0012 &2.341  &0.0035 &  1.903   &0.0035& 2.341\\
 $-2 \log L$ 	&1211.8 &1212.7 & 1119.3 &1132.8 &1026.2 & 1048.3   &1024.2    &1132.8\\
   AIC		&1229.8 &1230.7& 1141.3 &1154.9  &1052.2 &  1074.3  &1054.2 &1162.9\\
 \hline
\end{tabular}
